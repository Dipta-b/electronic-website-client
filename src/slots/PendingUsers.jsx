...
const PendingUsers = () => {
  ...
  useEffect(() => {
    const fetchPendingUsers = async () => {
      try {
        const res = await fetch(`${import.meta.env.DEV ? "http://localhost:5000" : "https://electronic-website-server.vercel.app"}/users/pending`, {
          credentials: "include",
        });
...
const handleApproveToBecameAdmin = async(userId, userName) => {
  ...
  try {
    const res = await fetch(`${import.meta.env.DEV ? "http://localhost:5000" : "https://electronic-website-server.vercel.app"}/users/approve/${userId}`,{
      method:'PATCH',
      credentials:'include',
    });
    const data = await res.json();
...
const handleReject = async (id) => {
  try {
    const res = await fetch(`${import.meta.env.DEV ? "http://localhost:5000" : "https://electronic-website-server.vercel.app"}/users/reject/${id}`, {
      method: "DELETE",
      credentials: "include",
    });
